// ============================================================
// 🛠️ ADMIN.JS — АДМИН-ПАНЕЛЬ (Firebase)
// ============================================================
...
function initPanel() {
  ...
  if (addStudentForm) {
    addStudentForm.addEventListener('submit', async (e) => {
      ...
      const passwordInput = document.getElementById('new-st-password').value.trim();
      ...
      if (passwordInput) {
        if (passwordInput.length < 8) {
          showModal('Пароль должен быть минимум 8 символов.', { title: 'Ошибка', icon: '⚠️' });
          return;
        }
        const result = await registerStudent(id, passwordInput, data);
        if (!result.success) {
...
async function renderStats() {
  ...
  const premiumCount = keys.filter(k => students[k].premium).length;
  ...
  const incomePremium = premiumCount * 40;
    ...
    <div class="request-card">
      <div style="display:flex; gap:14px; align-items:flex-start;">
        ...
        <div class="request-info">
          ...
          <small>🔐 Пароль: ${p.password ? 'задан' : 'не задан'}</small>
...
window.approveRequest = async (studentId) => {
  ...
  showModal('Одобрить заявку от ' + req.name + '?\n\nНомер: ' + req.id + '\n📷 Фото будет удалено.', {
    ...
    onConfirm: async () => {
      ...
      if (req.password) {
        const result = await registerStudent(studentId, req.password, userData);
        if (!result.success) {
          showModal('Ошибка: ' + result.error, { title: 'Ошибка', icon: '⚠️' });
          return;
        }
      } else {
        await saveStudent(studentId, userData);
      }
  ...
  el.innerHTML = keys.map(id => {
    ...
    return `
      <div class="list-item">
        <div>
          <b>${id}</b> — ${s.name}
          <br><small>${s.group} · ${s.spec || ''} ${s.premium ? '⭐' : ''}</small>
        </div>
        <div class="actions">
