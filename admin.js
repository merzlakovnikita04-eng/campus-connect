...
function initPanel() {
  ...
  if (addStudentForm) {
    addStudentForm.addEventListener('submit', async (e) => {
      ...
      e.target.reset();
      renderAll();
      showModal('Студент добавлен (без пароля — пароль студент задаёт сам через регистрацию).', {
        title: 'Готово!', icon: '✅'
      });
    });
  }
  ...
  if (addAdForm) {
    addAdForm.addEventListener('submit', async (e) => {
      ...
      if (result.success) {
        ...
        showModal('Не удалось добавить: ' + result.error + '\n\n(Правила Firestore пока запрещают запись в ads. Добавь вручную через консоль.)', {
          title: 'Ошибка', icon: '⚠️'
        });
      }
    });
