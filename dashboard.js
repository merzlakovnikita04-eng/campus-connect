// ============================================================
// 👤 DASHBOARD.JS — ЛИЧНЫЙ КАБИНЕТ (v16 — змейка)
// ============================================================
...
function renderProfile() {
  ...
  // Змейка-статус
  const snakeBadge = document.getElementById('snake-status-badge');
  if (snakeBadge) {
    if (student.snakeStatus) {
      snakeBadge.textContent = student.snakeStatus + ' · ' + (student.snakeBest || 0) + ' очков';
    } else {
      snakeBadge.textContent = '';
    }
  }
  ...
  modal.innerHTML = `
    <div class="modal-box" style="padding: 24px 20px;">
      <div class="student-profile-view">
        ${getAvatarHTML(s, 'big')}
        ${s.snakeStatus ? `<p style="margin:8px 0; font-size:14px; color:#fbbf24; font-weight:700;">${esc(s.snakeStatus)} · ${esc(s.snakeBest || 0)} очков</p>` : ''}
