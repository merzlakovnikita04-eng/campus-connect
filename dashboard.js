// ============================================================
// 👤 DASHBOARD.JS — ЛИЧНЫЙ КАБИНЕТ (v10 — пошаговый старт)
// ============================================================
...
onAuthStateChanged(auth, async (user) => {
  ...
  // ШАГ 2: заполнить профиль (только то, что не трогает другие коллекции)
  try {
    renderAvatar();
    renderProfile();
    checkPremium();
  ...
  // ШАГ 3: подгружаем остальное — каждая функция отдельно,
  // если падает — не рушит дашборд
  ...
  safeCall(updateFreeListingsInfo, 'счётчика объявлений');
});
