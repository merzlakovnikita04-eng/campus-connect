...
if (registerForm) {
  ...
  registerForm.addEventListener('submit', async (e) => {
    ...
    try {
      // 1. Получаем номер студенческого (через транзакцию)
      const newId = await getNextStudentNumber();

      // 2. Читаем фото как base64
      const reader = new FileReader();
      reader.onload = async (e) => {
        const photoBase64 = e.target.result;

        // 3. Создаём Auth-аккаунт + заявку в pending
      ...
      errEl.textContent = '❌ ' + (error.message || 'Произошла ошибка. Попробуйте позже.');
      submitBtn.disabled = false;
      submitBtn.textContent = 'Отправить заявку';
    }
  });
